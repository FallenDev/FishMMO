...
namespace FishMMO.Database.SqlServer.Services
{
	...
	public sealed class CharacterKnownAbilityService : BaseService<CharacterKnownAbilityEntity>, ICharacterKnownAbilityService
	{
		...
		/// </summary>
		private static readonly Func<SqlServerDbContext, long, CancellationToken, Task<List<CharacterKnownAbilityEntity>>> getKnownAbilitiesQuery =
			EF.CompileAsyncQuery((SqlServerDbContext context, long characterId, CancellationToken ct) =>
		...
		public CharacterKnownAbilityService(ISqlServerDbContextFactory dbContextFactory) : base(dbContextFactory)
		{
		}
