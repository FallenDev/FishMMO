...
namespace FishMMO.Database.SqlServer.Services
{
	...
	public sealed class CharacterEquipmentService : BaseService<CharacterEquipmentEntity>, ICharacterEquipmentService
	{
		...
		/// </summary>
		private static readonly Func<SqlServerDbContext, long, CancellationToken, Task<List<CharacterEquipmentEntity>>> getEquipmentQuery =
			EF.CompileAsyncQuery((SqlServerDbContext context, long characterId, CancellationToken ct) =>
		...
		public CharacterEquipmentService(ISqlServerDbContextFactory dbContextFactory) : base(dbContextFactory)
		{
		}
